...
import { createTransactions, updateTransactions } from "@/actions/transaction";
...
const AddTransactionForm = ({
  ...
  } = useFetch(editMode ? updateTransactions : createTransactions);
  ...
  useEffect(() => {
    if (transactionResult?.success && !transactionLoading) {
      ...
      reset();
      router.push(`/ac/${transactionResult.data.accountId}`);
    }
  ...
  const filteredCategories = category.filter((cate) => cate.type === type);
  return (
    <form className="space-y-2" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="flex items-center justify-between rounded-lg border mt-4 p-3">
      ...
      <div className="flex gap-4">
        ...
        <Button type="submit" className="w-full" disabled={transactionLoading}>
          {transactionLoading && <Loader2 className="animate-spin h-4 w-4" />}
